...
def on_message(client, userdata, msg):
    ...
    d = float(msg.payload.decode("utf-8"))
    ...
    if d >= 10000 :
        ...
        infot = pubClient.publish("control/led", "Red on")
        infot.wait_for_publish()
        print("*** [Red] light ON ***")
    elif 20 <= d < 40 :
        infot = pubClient.publish("control/led", "Yellow on")
        infot.wait_for_publish()
        print("*** [Yellow] light ON ***")        
    elif d < 20 :
        infot = pubClient.publish("control/led", "Green on")
        infot.wait_for_publish()
...
subClient.connect("192.168.0.93", 8883)
...
try:
    ...
    print("Finished!")
    subClient.unsubscribe("sensor/distance")
